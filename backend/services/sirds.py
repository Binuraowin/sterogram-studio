...
def create_text_depth_map(text: str, width: int, height: int) -> np.ndarray:
    """Render text as the 3D hidden shape."""
    ...
    font = None
    for size in range(min(width, height) // 3, 8, -2):
        for path in font_paths:
            try:
                f = ImageFont.truetype(path, size)
                bb = draw.textbbox((0, 0), display, font=f)
                if (bb[2] - bb[0]) <= width * 0.85 and (bb[3] - bb[1]) <= height * 0.45:
                    font = f
                    break
            except Exception:
                pass
    ...
    draw.text((x, y), display, fill=255, font=font)

    depth = gaussian_filter(np.array(img, dtype=np.float32), sigma=max(width, height) * 0.008)
    return depth / (depth.max() + 1e-9)


def generate_depth_map(hidden_object: str, width: int, height: int, hidden_object_type: str = "image") -> np.ndarray:
    """Generate depth map. Handles text type and falls back to a random shape for unknown names."""
    if hidden_object_type == "text":
        return create_text_depth_map(hidden_object, width, height)

    obj = hidden_object.lower().strip()
    fn = _SHAPE_MAP.get(obj)
    if fn:
        return fn(width, height)

    # Unknown shape: use HF to generate a real silhouette if available
    try:
        from services import hf_client
        if hf_client.is_available():
            return hf_client.generate_object_depth_map(hidden_object, width, height)
    except Exception:
        pass

    # No HF: fall back to text rendering
    return create_text_depth_map(hidden_object, width, height)
...
def _resolve_palette(color_mode: str) -> str:
    import random
    if color_mode == "random":
        return random.choice(list(PALETTES.keys()))
    return _COLOR_MODE_TO_PALETTE.get(color_mode, color_mode if color_mode in PALETTES else "rainbow")


def generate_stereogram(params: dict) -> tuple:
    """
    Wrapper for the stereograms router.
    Accepts params dict with keys: hidden_object, hidden_object_type, background_pattern,
    color_mode, depth_intensity, width, height.
    Returns (image: Image.Image, depth_map_img: Image.Image).
    """
    ...
    depth_intensity    = float(params.get("depth_intensity", 0.35))
    width              = int(params.get("width", 1080))
    height             = int(params.get("height", 1080))

    palette   = _resolve_palette(color_mode)
    max_shift = max(10, int(depth_intensity * 120))
    tile_w    = max(width // 7, 32)

    # ── Depth map ──────────────────────────────────────────────
    depth_map = generate_depth_map(hidden_object, width, height, hidden_object_type)

    # ── Texture: use HF-generated pattern if available ─────────
    texture = None
    if background_pattern:
        try:
            from services import hf_client
            if hf_client.is_available():
                texture = hf_client.generate_pattern_image(background_pattern, tile_w, height)
        except Exception:
            pass
    if texture is None:
        texture = generate_colorful_texture(tile_w, height, palette)

    # ── SIRDS ──────────────────────────────────────────────────
    actual_tile_w = texture.shape[1]
    actual_tile_h = texture.shape[0]
    xx    = np.arange(width,  dtype=np.int32)[np.newaxis, :]
    yy    = np.arange(height, dtype=np.int32)[:, np.newaxis]
    shift = (depth_map * max_shift).astype(np.int32)
    x_tex = (xx - shift) % actual_tile_w
    y_tex = yy % actual_tile_h
    result = texture[y_tex, x_tex]
    ...
    return stereogram_img, depth_img
